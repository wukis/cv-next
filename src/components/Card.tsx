...
export function Card<T extends React.ElementType = 'div'>({
  ...
  return (
    <Component
      className={clsx(className, 'group relative flex flex-col items-start scale-95 hover:scale-100 hover:-100 hover:bg-neutral-100/75 hover:dark:bg-neutral-800/50 rounded p-4')}
    >
      {children}
    </Component>
  )
...
Card.Eyebrow = function CardEyebrow<T extends React.ElementType = 'p'>({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center text-sm text-neutral-600 dark:text-neutral-300',
        decorate && 'pl-3.5',
      )}
      {...props}
    >
      {decorate && (
        <span
          ...
          aria-hidden="true"
        >
          <span className="h-4 w-0.5 rounded bg-neutral-200 dark:bg-neutral-500" />
        </span>
      )}
      {children}
    </Component>
